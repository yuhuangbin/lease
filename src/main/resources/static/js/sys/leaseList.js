...
function setFrom(jsonVal) {
    $.each(jsonVal, function (name, ival) {
        ...
        $("#leaseForm").find("select[name=" + name + "]").val(ival);
...
function validFrom(productType) {
	debugger
    if($("#leaseForm input[name='productCode']").val().length == 0){
        return "产品编码为必填项";
    }
    if($("#leaseForm input[name='buyTime']").val().length == 0){
        return "购买日期为必填项";
    }
    if($("#leaseForm input[name='factoryName']").val().length == 0){
        return "制作商为必填项";
    }if($("#leaseForm input[name='price']").val().length == 0){
        return "购买价格为必填项";
    }
	if(productType != 3) {
        if($("#leaseForm input[name='length']").val().length == 0){
            return "物品长度为必填项";
        }
        if($("#leaseForm input[name='with']").val().length == 0){
            return "物品宽度为必填项";
        }
        if($("#leaseForm input[name='high']").val().length == 0){
            return "物品高度为必填项";
        }
	} else {
        if($("#leaseForm input[name='power']").val().length == 0){
            return "功率为必填项";
        }
	}
	return "";
